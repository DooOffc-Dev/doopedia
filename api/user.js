...
export default async function handler(req, res) {
    ...
    // ==========================================
    // DEBUG: TEST ENDPOINT (CEK API HIDUP)
    // ==========================================
    if (req.query.route === 'test') {
        return res.status(200).json({
            status: 'success',
            message: 'API hidup dan berjalan!',
            timestamp: new Date().toISOString()
        });
    }

    // ==========================================
    // DATABASE CONNECTION (HANYA DIJALANKAN SAAT ROUTE TERKECUALI TEST)
    // ==========================================
    const db = await mysql.createConnection(dbConfig).catch(err => {
    ...
    if (route === 'register' && req.method === 'POST') {
        try {
            ...
            await db.end();
            return res.status(500).json({ status: 'error', message: 'Error di register: ' + error.message });
        }
    }
    ...
    if (route === 'login' && req.method === 'POST') {
        try {
            ...
            await db.end();
            return res.status(500).json({ status: 'error', message: 'Error di login: ' + error.message });
        }
    }
    ...
    if (route === 'profile' && req.method === 'GET') {
        try {
            ...
            await db.end();
            return res.status(500).json({ status: 'error', message: 'Error di profile: ' + error.message });
        }
    }
    ...
    if (route === 'balance' && req.method === 'GET') {
        try {
            ...
            await db.end();
            return res.status(500).json({ status: 'error', message: 'Error di balance: ' + error.message });
        }
    }
    ...
    if (route === 'change_api_key' && req.method === 'POST') {
        try {
            ...
            await db.end();
            return res.status(500).json({ status: 'error', message: 'Error di change_api_key: ' + error.message });
        }
    }
    ...
    if (route === 'change_password' && req.method === 'POST') {
        try {
            ...
            await db.end();
            return res.status(500).json({ status: 'error', message: 'Error di change_password: ' + error.message });
        }
    }

    await db.end();
    return res.status(400).json({ status: 'error', message: 'Route tidak valid: ' + route });
}
